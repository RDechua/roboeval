...
A mock env + mock base policy lets us check the action-composition path
end-to-end without lerobot or mujoco.
"""
...
def test_wrapper_reset_resets_base_policy():
    env, base, wrapper = _make_wrapper()
...
def test_wrapper_step_composes_base_plus_alpha_residual():
    ...
    # Expected env-received action: clamp(0.3 + 0.5 * 0.4, -1, 1) = 0.5
...
def test_wrapper_clamps_composed_action_to_action_space():
    ...
    # 0.9 + 0.5 * 1.0 = 1.4 → must clamp to 1.0
    wrapper.step(residual)
...
def test_wrapper_replaces_reward_with_custom_reward_fn():
    ...
    # Custom reward: always returns 7.0 regardless of info.
    wrapper = ResidualEnvWrapper(
        env=env,
        base_policy=base,
        compositor=compositor,
