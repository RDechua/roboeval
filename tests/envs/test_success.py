...
def test_below_threshold_never_succeeds():
    det = TransferCubeSuccessDetector(SuccessCriterion())
    for _ in range(20):
...
def test_dwell_fires_at_exact_step():
    crit = SuccessCriterion(z_threshold_m=0.05, dwell_steps=5)
...
def test_xy_outside_box_fails():
    crit = SuccessCriterion(xy_tolerance_m=0.05, target_xy=(0.0, 0.0))
    det = TransferCubeSuccessDetector(crit)
...
def test_xy_inside_box_succeeds():
    crit = SuccessCriterion(xy_tolerance_m=0.05, dwell_steps=2)
    det = TransferCubeSuccessDetector(crit)
...
def test_target_xy_offset():
    crit = SuccessCriterion(xy_tolerance_m=0.05, dwell_steps=1, target_xy=(0.5, 0.0))
    det = TransferCubeSuccessDetector(crit)
...
def test_reset_clears_counter():
    crit = SuccessCriterion(dwell_steps=3)
    det = TransferCubeSuccessDetector(crit)
    for _ in range(2):
        det.update(_cube_state(z=0.1))
